...
def changeOurppedFiles(stock):
    ...
    gm_mean = np.mean((df_new.iloc[:, 11], df_new.iloc[:, 12], df_new.iloc[:, 13], df_new.iloc[:, 14],
                      df_new.iloc[:, 15], df_new.iloc[:, 16]), axis=0)
    pr_mean = np.mean((df_new.iloc[:, 17], df_new.iloc[:, 18], df_new.iloc[:, 19], df_new.iloc[:, 20],
                       df_new.iloc[:, 21], df_new.iloc[:, 22]), axis=0)
    df_new.drop(df_new.iloc[:, 11:], inplace=True, axis=1)
