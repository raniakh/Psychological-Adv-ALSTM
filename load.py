...
def load_cla_data(data_path, tra_date, val_date, tes_date, seq=2, hinge=1,
    ...
    ins_ind = 0
    for date_ind in range(tra_ind, val_ind):  # from each day - for each stock takes a windows of "seq" days data
        # filter out instances without length enough history
        if date_ind < seq:
            continue
        for tic_ind in range(len(fnames)):
...
def load_cla_data_shuffle(data_path, tra_date, val_date, tes_date, seq=2, hinge=1,
                  date_format='%Y-%m-%d'):
    fnames = [fname for fname in os.listdir(data_path) if
              os.path.isfile(os.path.join(data_path, fname))]
    print(len(fnames), ' tickers selected')
    ...
    val_wd = np.zeros([val_num, seq, 5], dtype=float)
    val_gt = np.zeros([val_num, 1], dtype=float)
    traval_pv = np.zeros([tra_num+val_num, seq, fea_dim], dtype=float)
    traval_wd = np.zeros([tra_num+val_num, seq, 5], dtype=float)
    traval_gt = np.zeros([tra_num+val_num, 1], dtype=float)
    ins_ind=0
    for date_ind in range(tra_ind, tes_ind):
        if date_ind < seq:
            continue
        for tic_ind in range(len(fnames)):
            if abs(data_EOD[tic_ind][date_ind][-2]) > 1e-8 and \
                    data_EOD[tic_ind][date_ind - seq: date_ind, :].min() > -123320:
                traval_pv[ins_ind] = data_EOD[tic_ind][date_ind - seq: date_ind, : -2]
                traval_wd[ins_ind] = data_wd[date_ind - seq: date_ind, :]
                traval_gt[ins_ind] = (data_EOD[tic_ind][date_ind][-2] + 1) / 2
                ins_ind += 1
    indices_arr = np.array(range(tra_num+val_num))
    np.random.shuffle(indices_arr)

    traval_pv_shuffled = traval_pv[indices_arr, :, :]
    traval_wd_shuffled = traval_wd[indices_arr, :, :]
    traval_gt_shuffled = traval_gt[indices_arr, :]

    tra_pv = traval_pv_shuffled[0: tra_num, :, :]
    tra_wd = traval_wd_shuffled[0: tra_num, :, :]
    tra_gt = traval_gt_shuffled[0: tra_num, :]

    val_pv = traval_pv_shuffled[tra_num:, :, :]
    val_wd = traval_wd_shuffled[tra_num:, :, :]
    val_gt = traval_gt_shuffled[tra_num:, :]
